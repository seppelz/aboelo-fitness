...
const ExerciseVideo: React.FC<ExerciseVideoProps> = ({ exercise, onExerciseComplete }) => {
  ...
  // Theme und Responsive Design - muss vor den State-Variablen definiert werden
  ...
  const [instructionsExpanded, setInstructionsExpanded] = useState(!isMobile); // Standardmäßig geöffnet auf Desktop, geschlossen auf Mobil
  
  // Video-ID für Cloudinary
  ...
  // URL und Typ für Cloudinary Video
  ...
  const handleVideoError = () => {
    setError('Fehler beim Laden des Videos. Bitte versuchen Sie es später erneut.');
    setIsLoading(false);
  };
  ...
  const exerciseTitle = exercise.name || 'Übung';
          ...
          <Box sx={styles.videoBox}>
            ...
            {isVideoPlaying ? (
              ...
              <Card sx={{ maxWidth: '100%', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 15px rgba(0, 0, 0, 0.1)' }}>
                <CardMedia
                  component="img"
                  image={getVideoDetails().poster || `/images/exercise-placeholder.jpg`}
